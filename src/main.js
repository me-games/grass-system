/* --- Genex port glue (boot only — the author's code below is untouched) ---- */
...
/* --- Genex port glue: the bundled demo GLBs (Rusty Car / Porsche 911, ~25MB)
   are not shipped with this port, so hide the bundled-model dropdown. The
   "Load Model" toggle and "📂 Import GLB…" (your own .glb) still work. ------ */
gui.controllersRecursive().forEach((c) => {
  if (c._name === 'Model' && c.property === 'active') c.hide();
});
